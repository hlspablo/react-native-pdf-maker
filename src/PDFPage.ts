...
export type PageActions = TextAction | RectangleAction | ImageAction;

export type PageAction = {
  pageIndex?: number; // Not allowed in created pages
  mediaBox?: {
    // Not allowed for modified pages
    x: number;
    y: number;
    width: number;
    height: number;
...
export default class PDFPage {
  ...
  static modify = (pageIndex: number) => {
    const newPage = new PDFPage();
  ...
  drawImage = (
    ...
    if (
      typeof options.imageSource !== 'undefined' &&
      !['assets', 'path'].includes(options.imageSource)
    ) {
      throw new Error(
        'Only images from "assets" and "path" are currently supported!'
      );
    }
