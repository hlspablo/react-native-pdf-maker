...
RCT_EXPORT_METHOD(getDocumentsDirectory
...
RCT_EXPORT_METHOD(unloadAsset
                  : (NSString *)assetName destPath
                  : (NSString *)destPath resolverUnloadAsset
                  : (RCTPromiseResolveBlock)resolve rejecterUnloadAsset
                  : (RCTPromiseRejectBlock)reject) {
  reject(@"error",
         @"PDFLib.unloadAsset() is only available on Android. Try "
         @"PDFLib.getAssetpath().",
         nil);
}

RCT_EXPORT_METHOD(getAssetPath
                  : (NSString *)assetName resolverUnloadAsset
                  : (RCTPromiseResolveBlock)resolve rejecterUnloadAsset
                  : (RCTPromiseRejectBlock)reject) {
  resolve([[NSBundle mainBundle] pathForResource:assetName ofType:nil]);
}

RCT_EXPORT_METHOD(measureText
                  : (NSString *)text fontName
                  : (NSString *)fontName fontSize
                  : (NSInteger *)fontSize resolverMeasureText
                  : (RCTPromiseResolveBlock)resolve rejecterMeasureText
                  : (RCTPromiseRejectBlock)reject) {
  try {
    PDFWriter pdfWriter;
    ...
    PDFUsedFont::TextMeasures measures =
        font->CalculateTextDimensions(text.UTF8String, (long)fontSize);
    NSDictionary *result = [[NSDictionary alloc]
        initWithObjectsAndKeys:@(measures.width), @"width", @(measures.height),
                               @"height", nil];
    resolve(result);
...
RCT_EXPORT_METHOD(test
  ...
  // Open new PDF
  NSString *pdfPath =
      [NSString stringWithFormat:@"%@/%@", documentsDir, @"test.pdf"];
  PDFWriter pdfWriter;
  ...
  NSString *fontPath = [[NSBundle mainBundle] pathForResource:@"Times New Roman"
                                                       ofType:@".ttf"];
  NSLog(@"\n---\n%@\n---", fontPath);
  PDFUsedFont *font = pdfWriter.GetFontForFile(fontPath.UTF8String);
  AbstractContentContext::TextOptions textOptions(
      font, 14, AbstractContentContext::eGray, 0);
  ...
  PageContentContext *contentContext2 =
      pdfWriter.StartPageContentContext(page2);
  AbstractContentContext::GraphicOptions pathFillOptions2(
      AbstractContentContext::eFill, AbstractContentContext::eCMYK, 0xFF00FF);
  ...
  PageContentContext *contentContext3 =
      pdfWriter.StartPageContentContext(page3);
  contentContext3->q();
