...
void PDFPageFactory::endContext() {
  ...
  if (this->page != nullptr) {
    ...
    throw std::invalid_argument(
        @"No pages found - this should never happen!".UTF8String);
  }
}

void PDFPageFactory::createAndWrite(PDFWriter *pdfWriter,
                                    NSDictionary *pageActions) {
  ...
  page->SetMediaBox(PDFRectangle(coords.a.intValue, coords.b.intValue,
                                 dims.a.intValue, dims.b.intValue));
  factory.applyActions(pageActions[@"actions"]);
  factory.endContext();
  pdfWriter->WritePageAndRelease(page);
}

void PDFPageFactory::modifyAndWrite(PDFWriter *pdfWriter,
                                    NSDictionary *pageActions) {
...
// Add a FormXObject to the PDFWriter for the image specified in the given
// `pdfImageActions`.
  ...
  if ([imageType isEqualToString:@"png"]) {
    if (![[NSFileManager defaultManager] fileExistsAtPath:imagePath]) {
      NSString *msg = [NSString
          stringWithFormat:@"%@%@", @"No image found at path: ", imagePath];
    ...
    EStatusCodeAndObjectIDTypeList result =
        pdfWriter->CreateFormXObjectsFromPDF(
            (IByteReaderWithPosition *)&imageStream, PDFPageRange(),
            ePDFPageBoxMediaBox);
    ...
    formXObjectMap.insert(
        std::pair<NSString *, unsigned long>(imagePath, result.second.front()));
  }
}
...
void PDFPageFactory::drawText(NSDictionary *textActions) {
  ...
  AbstractContentContext::TextOptions textOptions(
      font, fontSize, AbstractContentContext::eRGB, hexColor);
  context->WriteText(coords.a.intValue, coords.b.intValue, value.UTF8String,
                     textOptions);
}
...
void PDFPageFactory::drawRectangle(NSDictionary *rectActions) {
  ...
  AbstractContentContext::GraphicOptions options(
      AbstractContentContext::eFill, AbstractContentContext::eRGB, hexColor);
  context->DrawRectangle(coords.a.intValue, coords.b.intValue, dims.a.intValue,
                         dims.b.intValue, options);
}
  ...
  if ([imageType isEqualToString:@"jpg"]) {
    ...
    if (dims.a && dims.b) {
      options.transformationMethod =
          AbstractContentContext::EImageTransformation::eFit;
    ...
    context->DrawImage(coords.a.intValue, coords.b.intValue,
                       imagePath.UTF8String, options);
...
void PDFPageFactory::drawImageAsPDF(NSDictionary *imageActions) {
  ...
  if ([imageType isEqualToString:@"jpg"] ||
    ...
    if (options.transformationMethod == AbstractContentContext::eMatrix) {
      ...
      if (options.fitPolicy == AbstractContentContext::eAlways) {
        ...
        scaleY = options.boundingBoxHeight / [image size].height;
                 ...
                 [image size].height > options.boundingBoxHeight) { // Overflow
        scaleX = [image size].width > options.boundingBoxWidth
                     ? options.boundingBoxWidth / [image size].width
                     : 1;
        scaleY = [image size].height > options.boundingBoxHeight
                     ? options.boundingBoxHeight / [image size].height
                     : 1;
      }
    ...
    // Retrieve & use the formXObject that we previously generated for this
    // image
    std::string formXObjectName =
        getResourcesDict()->AddFormXObjectMapping(formXObjectMap.at(imagePath));

    // Draw on the page's context
    context->q();
    context->cm(transformation[0], transformation[1], transformation[2],
...
NumberPair PDFPageFactory::getNumberKeyPair(NSDictionary *map, NSString *key1,
                                            NSString *key2) {
  NSNumber *a = nil;
  NSNumber *b = nil;
